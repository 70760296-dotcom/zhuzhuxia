...
FARM_COORDS = {
    ...
    # --- 吉林 ---
    '农安': (124.975026, 44.509817), '双辽': (123.51, 43.94), '通榆': (122.261505, 44.631419),
    '前郭': (124.276765, 44.594600), '双阳区': (125.781862, 43.667300), '大安市': (123.39, 45.38),
    # --- 河北 ---
    '广宗': (115.20, 37.19), '馆陶': (115.218494, 36.510758), '海兴': (117.377262, 38.030950),
    '新河': (115.281378, 37.434621), '冀州市': (115.54, 37.33), '辛集市': (115.30, 37.65),
    '宁晋县': (115.171980, 37.612749), '景县': (115.945897, 37.612491), '沽源县': (115.42, 41.62),
    '衡水': (115.82, 37.74), '枣强县': (115.83, 37.50), '南皮县': (116.943916, 38.064194),
    # --- 安徽 ---
...
def extract_weight_smart(text):
    ...
    try:
        num = float(text)
        if num > 50: # 排除可能是头数的情况
            # 按区间划分
            if num < 90: return "<90kg"
            elif num < 110: return "90-110kg"
            elif num < 130: return "110-130kg"
            else: return ">130kg"
    except:
        pass
...
if st.session_state.current_page == 'home':
    st.markdown("""<div style="text-align: center; padding: 40px 0;"><h1 style="font-size: 3rem; color: #FF4B4B; margin-bottom: 10px;">🐷 猪猪侠全力冲杀！</h1><p style="font-size: 1.2rem; color: #666;">牧原生猪产业链智能决策系统 v21.0</p><p style="color: #999;">""" + OCR_STATUS + """</p></div>""", unsafe_allow_html=True)
    st.markdown("---")
    ...
    if uploaded_files:
        try:
            ...
            with st.expander("⚙️ 列名配置", expanded=False):
                ...
                # 新增：体重列选择
                sel_weight = st.selectbox("体重(公斤)", ["无"] + [c for c in cols if '重' in c or 'kg' in c.lower()])

            df.rename(columns={sel_cust:'客户姓名', sel_mark:'屠宰场', sel_heads:'总头数', sel_sub:'子公司'}, inplace=True)
            if sel_price != "无": df.rename(columns={sel_price:'单价'}, inplace=True)
            else: df['单价'] = 0
            ...
            # --- 处理体重段 ---
            if sel_weight != "无":
                df['体重段'] = df[sel_weight].apply(extract_weight_smart)
            else:
                df['体重段'] = '未知'
            ...
            if not selected_markets:
                ...
                # --- 2. 趋势分析 (新增体重段) ---
                st.markdown("#### 📊 屠宰场接收趋势分析")
                
                # 2.1 每日接收数量变化
                daily_trend = df_view.groupby('日期')['总头数'].sum().reset_index()
                fig_line = px.line(daily_trend, x='日期', y='总头数', markers=True, title="每日总接收量走势")
                st.plotly_chart(fig_line, use_container_width=True)
                
                # 2.2 每日体重段分布变化 (堆叠柱状图)
                if sel_weight != "无":
                    st.markdown("##### 每日接收体重段分布")
                    weight_trend = df_view.groupby(['日期', '体重段'])['总头数'].sum().reset_index()
                    fig_bar = px.bar(weight_trend, x='日期', y='总头数', color='体重段', 
                                     title="每日各体重段接收量", barmode='stack')
                    st.plotly_chart(fig_bar, use_container_width=True)
                else:
                    st.info("未配置体重列，无法显示体重段分布。")
                
                # 2.3 市场份额
                st.markdown("#### 📉 市场份额")
                fig_pie = px.pie(stats, values='总量', names='屠宰场', title='市场份额', hole=0.4)
                ...
                if not mid.empty:
                    ...
                    cust['距离'] = mid.groupby('客户姓名')['运距'].first().values
